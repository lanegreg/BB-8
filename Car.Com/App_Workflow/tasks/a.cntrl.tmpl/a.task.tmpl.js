/**  {controller-name} {action-name} Tasks  **/
...
///- JS tasks
function processTemplatesFor(deviceGroup) {
  var path = templatesFolder + deviceGroup;
  ...
  fs.exists(path, function (exists) {
    if (exists) {
      ...
      for (var prop in tmpls) {
        code += 'module.exports.' + prop.toString() + '=' + tmpls[prop].toString() + ';';
      }
...
gulp.task(filename + '-compiletmpls', function (done) {
  processTemplatesFor('mobi');
  processTemplatesFor('tabl');
  processTemplatesFor('desk');
